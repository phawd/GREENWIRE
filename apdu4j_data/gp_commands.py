...
class GPCommand:
    ...
    def install_for_load(package_aid: Union[str, bytes], 
        ...
        Args:
            package_aid: Package AID to install
            security_domain_aid: Target security domain (optional)
        """
        ...
        if security_domain_aid is None:
            sd_aid_bytes = b''
        elif isinstance(security_domain_aid, str):
            sd_aid_bytes = bytes.fromhex(security_domain_aid.replace(' ', ''))
        else:
            sd_aid_bytes = security_domain_aid
        
        # Build install data: [pkg_aid_len][pkg_aid][sd_aid_len][sd_aid][hash_len][hash][param_len][param]
        data = bytes([len(pkg_aid_bytes)]) + pkg_aid_bytes
        data += bytes([len(sd_aid_bytes)]) + sd_aid_bytes
        data += b'\x00'  # No hash
        data += b'\x00'  # No install parameters
        
        return APDU4JCommand(GP_CLA, GP_INS_INSTALL, GP_INSTALL_FOR_LOAD, 0x00, data)
    
    @staticmethod
    def load_chunk(chunk_data: bytes, is_last: bool = False) -> APDU4JCommand:
        """Load CAP file chunk.
        
        Args:
            chunk_data: Chunk of CAP file data
            is_last: True if this is the last chunk
        """
        p1 = 0x00 if is_last else 0x80  # 0x80 = more data, 0x00 = last
        p2 = len(chunk_data)
        
        return APDU4JCommand(GP_CLA, GP_INS_LOAD, p1, p2, chunk_data)
    
    @staticmethod
    def install_for_install(package_aid: Union[str, bytes], 
                           applet_aid: Union[str, bytes],
                           instance_aid: Union[str, bytes],
                           privileges: int = 0x00,
        ...
        # Build install data
        data = bytes([len(pkg_aid_bytes)]) + pkg_aid_bytes
        data += bytes([len(app_aid_bytes)]) + app_aid_bytes  
        data += bytes([len(inst_aid_bytes)]) + inst_aid_bytes
        data += bytes([1, privileges])  # Privileges length + value
        data += bytes([len(install_params)]) + install_params
        data += b'\x00'  # No install token
        
        return APDU4JCommand(GP_CLA, GP_INS_INSTALL, 
                           GP_INSTALL_FOR_INSTALL | GP_INSTALL_FOR_MAKE_SELECTABLE, 
                           0x00, data)
        """Install for Load command.
