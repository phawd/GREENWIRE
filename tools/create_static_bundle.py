...
def collect_static(target: Path = BUNDLE_OUT):
    ...
    # Copy top-level lib jars (e.g., GlobalPlatformPro.jar) if present
    lib_dir = ROOT / 'lib'
    if lib_dir.exists():
        lib_out = target / 'lib'
        shutil.copytree(lib_dir, lib_out, dirs_exist_ok=True)
        print("  Copied lib/ JARs to bundle")

    # Copy SDK javacard lib (tools.jar, api jars, export files) if present
    sdk_jc_lib = ROOT / 'sdk' / 'javacard' / 'lib'
    if sdk_jc_lib.exists():
        sdk_out = target / 'sdk' / 'javacard' / 'lib'
        shutil.copytree(sdk_jc_lib, sdk_out, dirs_exist_ok=True)
        print("  Copied sdk/javacard/lib contents (tools.jar, api jars) into bundle")

    # Copy static/java artifacts (ant-javacard.jar, gp.jar) if present
    static_java = ROOT / 'static' / 'java'
    if static_java.exists():
        shutil.copytree(static_java, target / 'static' / 'java', dirs_exist_ok=True)
        print("  Copied static/java artifacts (ant-javacard.jar, gp.jar) into bundle")
...
""")
    ...
    # Add a short README describing required JavaCard offline artifacts
    readme = target / 'JAVACARD_OFFLINE_NOTICE.txt'
    readme.write_text("""JavaCard offline setup notice

This static bundle includes available helper jars (ant-javacard.jar) when present.

Important missing artifacts that may be required for convertCap/deployCap:
 - sdk/javacard/lib/tools.jar  (JavaCard converter runtime)
 - lib/GlobalPlatformPro.jar   (GlobalPlatformPro fat JAR for CAP deployment)

Place the above files into the indicated paths inside the bundle if you need full offline
convert/deploy capability. See GREENWIRE/docs/JAVACARD_OFFLINE_SETUP.md for steps to
obtain them and licensing notes.
""")
    print("  Wrote JAVACARD_OFFLINE_NOTICE.txt into bundle")
