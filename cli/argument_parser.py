...
Examples:
  ...
  python greenwire.py install-cap myapplet.cap
    ...
    _add_configuration_parser(subparsers)
...
def _add_testing_parser(subparsers):
    ...
    vuln_scan = testing_sub.add_parser("vuln-scan", help="Run vulnerability checks against CAP/GP artifacts")
    vuln_scan.add_argument("--card-file", type=str, help="Card JSON file containing state and keys")
    vuln_scan.add_argument("--cap-file", type=str, help="CAP file path for static analysis")
    vuln_scan.add_argument("--gp-binary", type=str, help="GlobalPlatform binary path for hashing")
    vuln_scan.add_argument("--suite", nargs="+", help="Specific vulnerability tests to run")
    vuln_scan.add_argument("--run-pos", action="store_true", help="Execute POS regression prior to scan")
    vuln_scan.add_argument("--run-atm", action="store_true", help="Execute ATM regression prior to scan")
    vuln_scan.add_argument("--include-hsm", action="store_true", help="Include HSM command simulation")
    vuln_scan.add_argument("--output", type=str, help="Write results to a JSON file")
...
def _add_easycard_parser(subparsers):
    ...
    # Profile selection & listing
    realworld.add_argument("--profile", type=str,
              help="Use a predefined card profile ID")
    ...
    realworld.add_argument("--list-variants", action="store_true",
              help="List variants available for the selected profile(s)")
    ...
    realworld.add_argument("--cardholder-name", type=str, help="Override cardholder name")
    realworld.add_argument("--expiry-date", type=str, help="Expiry date in MM/YY format")
    realworld.add_argument("--preferred-bank", type=str, help="Preferred issuing bank identifier")
    realworld.add_argument("--force-bin", type=str, help="Force BIN/IIN prefix for generated PANs")
    realworld.add_argument("--terminal-profile", type=str, help="Terminal profile ID to bind to the issued card")
    realworld.add_argument("--list-terminal-profiles", action="store_true",
              help="List available terminal/ATM configuration profiles and exit")
    realworld.add_argument("--terminal-overrides", type=str,
              help="JSON string of overrides applied on top of the terminal profile")

    # Output and CAP handling
    realworld.add_argument("--generate-cap", action="store_true",
              help="Generate JavaCard CAP file for each issued card")
    realworld.add_argument("--cap-output-dir", type=str, default="realworld_caps",
              help="Directory for generated CAP files")
    realworld.add_argument("--test-merchant", action="store_true",
              help="Run merchant POS regression immediately after issuance")
    realworld.add_argument("--test-atm", action="store_true",
              help="Run ATM regression immediately after issuance")
    realworld.add_argument("--production-ready", action="store_true",
              help="Force production settings (DDA, conservative risk, logging)")
    realworld.add_argument("--output-file", type=str, help="Aggregate output file for generated cards")
    realworld.add_argument("--output-format", choices=["json", "csv", "text"], default="json",
              help="Format for aggregate output when --output-file is used")
...
def _add_configuration_parser(subparsers):
    """Add configuration subcommand and its options."""
    config_parser = subparsers.add_parser(
        "config",
        help="Inspect or modify GREENWIRE's central configuration",
    )
    config_parser.add_argument(
        "operation",
        choices=["show", "set", "list", "reset"],
    ...
    config_parser.add_argument(
        "--section",
        type=str,
        help="Configuration section to list/reset",
    )
    config_parser.add_argument(
        "--interactive",
        action="store_true",
