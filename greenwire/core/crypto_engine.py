...
def aes_decrypt(key: bytes, ciphertext: bytes, iv: bytes) -> bytes:
    ...
    try:
        ...
        logger.info(
            "AES decrypt complete | mode=CBC key_len=%d duration_ms=%.3f",
            len(key) * 8,
            duration_ms,
        )
        return plaintext
        ...
        duration_ms = (time.perf_counter() - start) * 1000.0
        logger.error(
            "AES decrypt failed | mode=CBC key_len=%d duration_ms=%.3f error=%s",
            len(key) * 8,
            duration_ms,
            e,
            exc_info=True,
...
def aes_gcm_decrypt(key: bytes, ciphertext: bytes, tag: bytes, iv: bytes, aad: bytes = b"") -> bytes:
    ...
    try:
        ...
        duration_ms = (time.perf_counter() - start) * 1000.0
        logger.info(
            "AES-GCM decrypt complete | key_len=%d duration_ms=%.3f aad_len=%d",
            len(key) * 8,
            duration_ms,
            len(aad),
        )
        ...
        duration_ms = (time.perf_counter() - start) * 1000.0
        logger.error(
            "AES-GCM decrypt failed | key_len=%d duration_ms=%.3f aad_len=%d error=%s",
            len(key) * 8,
            duration_ms,
            len(aad),
            e,
