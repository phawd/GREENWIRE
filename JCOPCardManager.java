...
public class JCOPCardManager {
     ...
     * @throws CardException if connection fails
     */
    public void connect() throws CardException {
        try {
            card = terminal.connect("T=1");
            channel = card.getBasicChannel();
            logger.log(Level.INFO, "Connected to card: {0}", new Object[] { card });
        } catch (CardException e) {
            logger.log(Level.SEVERE, "Connection failed: {0}", new Object[] { e.getMessage() });
            throw new CardException("Connection failed: " + e.getMessage());
        }
    }
    ...
    /**
     * Search for root CA certificates
     * @param commandType the type of command (e.g., "DDA", "SDA")
    ...
    private void sendAPDU(byte[] apdu) throws CardException {
        ...
        try {
            ...
            logger.log(Level.SEVERE, "Failed to send APDU: {0}", new Object[] { e.getMessage() });
            throw new CardException("Failed to send APDU: " + e.getMessage());
        }
    }

    /**
     * Generate a random 16-digit LUN
     * @return random LUN string
     */
     ...
     * @throws CardException if disconnection fails
     */
