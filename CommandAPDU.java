...
public final class CommandAPDU {
    ...
    public CommandAPDU(String apdu) {
        this(hexStringToByteArray(apdu));
    }

    private static byte[] hexStringToByteArray(String s) {
        if (s == null || (s.length() & 1) != 0) {
            throw new IllegalArgumentException("APDU string must have even length");
        }
        int len = s.length();
        ...
        for (int i = 0; i < len; i += 2) {
            ...
            if (hi < 0 || lo < 0) {
                throw new IllegalArgumentException("Invalid hex character in APDU string");
            }
