...
class EMVProcessor:
    ...
    def build_pdol(self, pdol_template: str, terminal_data: dict = None) -> str:
        """Build PDOL data using configurable terminal data."""
        if terminal_data is None:
            terminal_data = {
                '9F33': 'E0F0C8',  # Terminal Capabilities
                '9F35': '22',      # Terminal Type
                '9F40': 'A000000003',  # Additional Terminal Capabilities
                '9F1A': '0250',    # Terminal Country Code
                '5F2A': '0978',    # Transaction Currency Code
                '9A': '250926',    # Transaction Date (YYMMDD)
                '9C': '00',        # Transaction Type
                '9F37': '11223344' # Unpredictable Number
            }
        from core.tlvlib import TLVLib
        pdol_tags = []
        i = 0
        pdol_bytes = binascii.unhexlify(pdol_template)
        while i < len(pdol_bytes):
            ...
            length = pdol_bytes[i]
            i += 1
            pdol_tags.append((tag_str, length))
        pdol_data = b''
        for tag, length in pdol_tags:
            value = terminal_data.get(tag, '00' * length)
            value_bytes = bytes.fromhex(value) if isinstance(value, str) else value
            if len(value_bytes) < length:
                value_bytes = value_bytes.ljust(length, b'\x00')
            pdol_data += value_bytes[:length]
        return pdol_data.hex().upper()