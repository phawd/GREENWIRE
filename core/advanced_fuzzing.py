...
class MemoryExtractionFuzzer:
    ...
    def setup_logging(self):
        ...
        log_name = self.log_name or f'advanced_fuzzing_{timestamp}.log'
        log_dir = self.log_directory or Path.cwd()
        ...
        if self.gw_logger:
            try:
                self.gw_logger.info(f"Advanced fuzzing logger initialized at {log_file}", "advanced_fuzzing")
            except Exception:
                pass
    ...
    def log_verbose(self, message: str, level='info'):
        ...
        if self.enable_logging and self.logger:
            ...
            log_method(message)
        if getattr(self, 'gw_logger', None):
            gw_method = getattr(self.gw_logger, level, None)
            if callable(gw_method):
                try:
                    gw_method(message, "advanced_fuzzing")
                except Exception:
                    pass
