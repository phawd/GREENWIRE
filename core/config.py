...
@dataclass
class RoleConfig:
    """Operational role configuration for merchant/ATM/issuer flows."""

    active_role: str = "issuer"
    profiles: Dict[str, Dict[str, Any]] = field(
        default_factory=lambda: {
            "merchant": {
                "hardware_reader": "PCSC:MERCHANT",
                "hardware_notes": "Primary PC/SC reader for merchant terminal emulation",
                "gp_transport": "pcsc",
                "jcop_target": "/dev/ttyUSB0",
                "mifare_interface": "nfc-a",
                "log_file": "logs/merchant_session.log",
                "auto_enable_logging": True,
            },
            "atm": {
                "hardware_reader": "PCSC:ATM",
                "hardware_notes": "Dedicated PC/SC reader or ATM simulator port",
                "gp_transport": "pcsc",
                "jcop_target": "/dev/ttyUSB1",
                "mifare_interface": "nfc-b",
                "log_file": "logs/atm_session.log",
                "auto_enable_logging": True,
            },
            "issuer": {
                "hardware_reader": "PCSC:ISSUER",
                "hardware_notes": "Issuer personalization station",
                "gp_transport": "pcsc",
                "jcop_target": "127.0.0.1:9025",
                "mifare_interface": "nfc-f",
                "log_file": "logs/issuer_session.log",
                "auto_enable_logging": True,
            },
        }
    )
...
class ProvisioningConfig:
    """Provisioning pipeline configuration for secure card material loading."""

    input_file: str = "input.json"
    decrypted_cache: str = os.path.join("temp", "decrypted_input.json")
    ...
    def from_file(cls, config_path: str) -> 'GreenwireConfig':
        ...
        try:
            ...
            return cls(
                ...
                roles=RoleConfig(**data.get('roles', {})),
                provisioning=ProvisioningConfig(**data.get('provisioning', {}))
            )
