...
from core.ui.menu_actions import (
    ...
    show_smartcard_javacard_menu,
    show_crypto_menu,
    show_fuzzing_vulnerability_menu,
    show_hardware_device_menu,
    show_utilities_menu,
    show_atm_hsm_menu
)
...
# ---------------------------------------------------------------------------
# Path setup (REGRESSION FIX)
# Historically this project used flat imports like `from core.xyz import ...`.
# That requires the literal GREENWIRE/core directory itself to be on sys.path.
# A recent refactor removed that insertion and only added the inner package
# path (GREENWIRE/greenwire), breaking all `core.*` imports. We restore the
# original behavior while still supporting the namespaced package imports
# (greenwire.core.*) used by newer modules.
# ---------------------------------------------------------------------------
...
# Keep inner package on path for explicit package-style imports
_INNER_PKG_DIR = os.path.join(_BASE_DIR, 'greenwire')
if os.path.isdir(_INNER_PKG_DIR) and _INNER_PKG_DIR not in sys.path:
    sys.path.insert(0, _INNER_PKG_DIR)
...
def adb_cmd(adb_args: list, restart: bool = True, timeout: int = 10, cache_restart: bool = True) -> dict:
    """Execute ADB command with optional server restart and timing metrics.

    Args:
        adb_args: ADB command arguments (without 'adb' prefix)
    ...
    if restart:
        ...
        if not cache_restart or (now - _LAST_ADB_RESTART) > ADB_RESTART_CACHE_SECONDS:
            try:
                # Use check=False so restart failures don't raise
                ...
                # ADB restart failures are non-fatal
                pass

    # Execute main command
    try:
        result = subprocess.run([adb] + list(adb_args or []), capture_output=True, text=True, timeout=timeout, check=False)
        ...
        try:
            cmd_preview = " ".join(str(x) for x in (adb_args or [])[:3])
        except Exception:
            cmd_preview = ""
...
# Prefer bundled JDK if present for gp.jar, converter, and apdu4j compatibility
try:
    prefer_static_jdk()
except Exception:
    pass
# ---------------------------------------------------------------------------
# Early role selection (Merchant / ATM / Issuer) prior to heavy imports
# This allows a streamlined startup path and future role-specific optimization.
# If a subcommand is already provided we skip the interactive role prompt.
# ---------------------------------------------------------------------------
def _early_role_selection():
    if any(a in sys.argv for a in ["--help", "-h"]) or len(sys.argv) > 1:
        return None
    print("\nGREENWIRE Operational Mode Selection")
    print("----------------------------------")
    print(" 1. Merchant (POS Terminal Emulation)")
    print(" 2. ATM (Cash Machine Emulation)")
    print(" 3. Issuer (Card Generation / Personalization)")
    print(" 4. Continue to full menu")
    choice = input("Select role [1-4]: ").strip() or "4"
    mapping = {"1": "merchant", "2": "atm", "3": "issuer", "4": None}
    return mapping.get(choice, None)

_SELECTED_ROLE = _early_role_selection()
...
def _start_crypto_fuzzing_session_fallback(config, verbose=True):
    """Fallback cryptographic fuzzing entrypoint — used only when the
    dynamically imported implementation is not available. This avoids
    redefining start_crypto_fuzzing_session when import_manager already
    provided it earlier in the file.
    """
    return {
...
# Install fallback only if a callable wasn't provided by the import manager
if not callable(globals().get('start_crypto_fuzzing_session')):
    start_crypto_fuzzing_session = _start_crypto_fuzzing_session_fallback
...
class NFCDaemon:
                ...
                while not self._stop_event.is_set():
                    ...
                    for device_id in new_devices:
                        ...
                        if nfc_info.get('status') == 'available':
                            ...
                            logger.info(
                                "Android device with NFC added: %s (%s %s) [%s]",
                                device_id,
                                nfc_info.get('brand', 'Unknown'),
                                nfc_info.get('model', 'Unknown'),
                                "NFC",
                            )
                    ...
                    for device_id in removed_devices:
                        ...
                        logger.info("Android device removed: %s [%s]", device_id, "DEVICE")
            else:
        ...
        try:
            for device in iter(self._monitor.poll, None):
                ...
                if device.action == 'add' and self._detect_android_device(device):
                    ...
                    logger.info("Android device with NFC detected: %s [%s]", device.get('DEVNAME'), "NFC")
                    ...
                    if device.get('DEVNAME') in self._android_devices:
                        ...
                        logger.info(
                            "Android device removed: %s [%s]",
                            device.get('DEVNAME'),
                            "DEVICE",
                        )
        except Exception as e:
            logger.error("Error in Android device monitoring: %s", e, "DEVICE_MONITOR")
    ...
    def _start_nfc_listener(self, device_path):
        ...
        try:
            ...
            logger.info(
                "NFC listener started for device: %s [%s]",
                device_path,
                "NFC_LISTENER",
            )
        except Exception as e:
            logger.error(
                "Error starting NFC listener for %s: %s",
                device_path,
                e,
                "NFC_LISTENER",
            )
    ...
    def _start_nfc_listener_android(self, device_id, nfc_info):
        ...
        try:
            logger.info("Starting Android NFC listener for %s [%s]", device_id, "NFC")
            android_verifier = AndroidNFCVerifier()
            test_result = android_verifier.test_nfc_functionality(device_id)
            if test_result.get('test_status') == 'functional':
                logger.info("Android NFC fully functional on %s [%s]", device_id, "NFC")
                logger.info(
                    "  Device: %s %s [%s]",
                    nfc_info.get('brand', 'Unknown'),
                    nfc_info.get('model', 'Unknown'),
                ...
                logger.info(
                    "  HCE Services: %s [%s]",
                    test_result.get('hce_services', 0),
                    "NFC",
                )
            else:
            ...
            logger.error(
                "Error starting Android NFC listener for %s: %s",
                device_id,
                e,
                "NFC_LISTENER",
            )
    ...
    def start(self):
        ...
        try:
            ...
            logger.error("Error starting NFC Daemon: %s", e, "NFC_DAEMON")
            self._thread = None
            return False
    ...
    def stop(self):
        ...
        try:
            ...
            logger.error("Error stopping NFC Daemon: %s", e, "NFC_DAEMON")
            self._thread = None
            return False
...
class AndroidNFCVerifier:
    ...
    def scan_connected_devices(self):
        ...
        try:
            ...
            if result["ok"]:
                ...
                logger.info(
                    "Found %s connected Android devices via ADB "
                    "(took %sms) [%s]",
                    len(devices),
                    result['timing_ms'],
                    "ADB",
            ...
            logger.error("Error scanning Android devices: %s", e, "ADB")
    ...
    def verify_nfc_capability(self, device_id):
        ...
        try:
            ...
            logger.info(
                "NFC verification for %s: %s (took %sms) [%s]",
                device_id,
                result['status'],
                result['timing_ms'],
                "NFC_VERIFY",
    ...
    def test_nfc_functionality(self, device_id):
        ...
        try:
            logger.info("Testing NFC functionality on %s... [%s]", device_id, "NFC_TEST")
    ...
    def enable_nfc_via_adb(self, device_id):
        ...
        try:
            logger.info("Attempting to enable NFC on %s via ADB [%s]", device_id, "NFC_ENABLE")
            results = []
            ...
            # Verify final status
            # use top-level `time` import; avoid re-importing locally
...
# Complete implementations for smartcard fuzzing classes
class SmartcardFuzzer:
    """Complete smartcard fuzzer with contactless and attack simulation capabilities."""
    ...
    def _log(self, message):
        ...
        if self.verbose:
            print(f"[SmartcardFuzzer] {message}")

    def fuzz_contactless(self, aids, iterations, ca_file):
        """
        Fuzz contactless smartcards with specified AIDs.

        Args:
            aids: List of Application Identifiers to test
            iterations: Number of fuzzing iterations per AID
            ca_file: Optional card file for additional context

        Returns:
            List of fuzzing results with responses
        """
        self._log(f"Starting contactless fuzzing: {len(aids)} AIDs, {iterations} iterations")
        results = []

        for aid_idx, aid in enumerate(aids, 1):
            self._log(f"Testing AID {aid_idx}/{len(aids)}: {aid}")

            for iteration in range(iterations):
                # SELECT AID command
                select_apdu = [0x00, 0xA4, 0x04, 0x00, len(aid)] + list(aid) + [0x00]

                # Simulate or send SELECT command
                select_response = self._send_apdu(select_apdu)

                # If SELECT successful, try GET PROCESSING OPTIONS
                gpo_response = None
                if select_response and select_response[-2:] == b'\x90\x00':
                    # Build GPO command (EMV standard)
                    pdol_data = b'\x83\x00'  # Empty PDOL for testing
                    gpo_apdu = [0x80, 0xA8, 0x00, 0x00, len(pdol_data)] + list(pdol_data) + [0x00]
                    gpo_response = self._send_apdu(gpo_apdu)

                # Try READ RECORD fuzzing
                read_results = []
                for sfi in range(1, 11):  # Test SFIs 1-10
                    for rec in range(1, 6):  # Test records 1-5
                        read_apdu = [0x00, 0xB2, rec, (sfi << 3) | 0x04, 0x00]
                        read_response = self._send_apdu(read_apdu)
                        if read_response and read_response[-2:] == b'\x90\x00':
                            read_results.append({
                                'sfi': sfi,
                                'record': rec,
                                'data': read_response[:-2]
                            })
                ...
                result = {
                    ...
                    'gpo': gpo_response,
                    'read_records': read_results,
                    'timestamp': time.time()
                }
                results.append(result)

        self._log(f"Fuzzing complete: {len(results)} results collected")
        self.results = results
        return results

    def simulate_attack_scenario(self, attack_type):
        """
        Simulate various EMV attack scenarios.

        Supported attack types:
        - wedge: EMV Wedge attack (force magnetic stripe fallback)
        - cvm_downgrade: CVM downgrade attack (force no CVM)
        - relay: Relay attack simulation
        - replay: Transaction replay attack
        - pin_harvest: PIN harvesting attempt

        Returns:
            Dictionary with attack simulation results
        """
        self._log(f"Simulating {attack_type} attack scenario")

        attack_scenarios = {
            'wedge': self._simulate_wedge_attack,
            'cvm_downgrade': self._simulate_cvm_downgrade,
            'relay': self._simulate_relay_attack,
            'replay': self._simulate_replay_attack,
            'pin_harvest': self._simulate_pin_harvest
        }

        if attack_type.lower() in attack_scenarios:
            return attack_scenarios[attack_type.lower()]()
        else:
            return {
    ...
    def _simulate_wedge_attack(self):
        """Simulate EMV Wedge attack (force magstripe fallback)."""
        return {
            'attack_type': 'wedge',
            'status': 'simulated',
            'method': 'CVM requirement manipulation',
            'steps': [
                'Send SELECT AID',
                'Intercept GPO response',
                'Modify CVM list to force magstripe',
                'Complete transaction with modified data'
            ],
            'success_rate': 0.45,
            'countermeasures': ['Online PIN', 'Terminal validation', 'CVM verification']
        }

    def _simulate_cvm_downgrade(self):
        """Simulate CVM downgrade attack."""
        return {
            'attack_type': 'cvm_downgrade',
            'status': 'simulated',
            'method': 'Cardholder Verification Method downgrade',
            'steps': [
                'Intercept GPO command',
                'Modify CDOL to remove CVM requirement',
                'Force "No CVM" or "Signature only"',
                'Complete transaction without PIN'
            ],
            'success_rate': 0.62,
            'countermeasures': ['Mandatory online auth', 'Floor limits', 'Random online']
        }

    def _simulate_relay_attack(self):
        """Simulate relay attack."""
        return {
            'attack_type': 'relay',
            'status': 'simulated',
            'method': 'Real-time communication relay',
            'steps': [
                'Capture APDU from terminal',
                'Relay to legitimate card',
                'Relay response back to terminal',
                'Complete transaction remotely'
            ],
            'success_rate': 0.78,
            'distance_limit': '50+ meters with RF equipment',
            'countermeasures': ['Distance bounding', 'Time measurement', 'Relay-resistant authentication']
        }

    def _simulate_replay_attack(self):
        """Simulate transaction replay attack."""
        return {
            'attack_type': 'replay',
            'status': 'simulated',
            'method': 'Transaction data replay',
            'steps': [
                'Capture transaction data',
                'Store ARQC and cryptogram',
                'Attempt replay at different terminal',
                'Check for duplicate detection'
            ],
            'success_rate': 0.15,
            'countermeasures': ['Transaction counters', 'Unpredictable numbers', 'Online verification']
        }

    def _simulate_pin_harvest(self):
        """Simulate PIN harvesting attack."""
        return {
            'attack_type': 'pin_harvest',
            'status': 'simulated',
            'method': 'PIN interception and collection',
            'steps': [
                'Present fake PIN pad',
                'Capture PIN entry',
                'Relay PIN to legitimate terminal',
                'Store PIN for later use'
            ],
            'success_rate': 0.35,
            'countermeasures': ['Encrypted PIN pad', 'PCI PTS certification', 'Tamper detection']
        }

    def _send_apdu(self, apdu):
        """
        Send APDU to card or simulate response.

        Args:
            apdu: List of bytes representing APDU command

        Returns:
            bytes: Response data including SW1 SW2
        """
        # If connected to real card, send actual APDU
        if self.connection:
            try:
                data, sw1, sw2 = self.connection.transmit(apdu)
                return bytes(data) + bytes([sw1, sw2])
            except Exception as e:
                self._log(f"Error sending APDU: {e}")
                return b'\x6F\x00'  # Technical error
        else:
            # Simulation mode: return realistic fake responses
            if apdu[1] == 0xA4:  # SELECT
                return b'\x6F\x1A\x84\x0E\xA0\x00\x00\x00\x03\x10\x10\xA5\x08\x88\x00\x00\x00\x00\x00\x00\x00\x90\x00'
            elif apdu[1] == 0xA8:  # GPO
                return b'\x77\x0A\x82\x02\x00\x00\x94\x04\x08\x01\x01\x00\x90\x00'
            elif apdu[1] == 0xB2:  # READ RECORD
                return b'\x70\x10\x5A\x08\x12\x34\x56\x78\x90\x12\x34\x56\x90\x00'
            else:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--non-interactive", action="store_true",
                        help="Disable interactive prompts; assume non-interactive defaults")
    parser.add_argument("--role", choices=["merchant", "atm", "issuer"], help="Directly set operational role (skip prompt)")
    parser.add_argument("--encrypted-input", dest="encrypted_input", type=str, help="Path to encrypted input.json containing sensitive personalization seeds")
    parser.add_argument("--input-passphrase", dest="input_passphrase", type=str, help="Passphrase to decrypt encrypted input file (optional; will prompt if omitted)")
    # Production-mode CLI overrides (useful for automation/non-interactive)
    parser.add_argument("--production-dataset", type=str, help="Name of production dataset to use for generation")
    parser.add_argument("--prod-iin", type=str, help="Override IIN/BIN prefix when using production dataset")
    parser.add_argument("--prod-enforce-luhn", action="store_true", help="Enforce Luhn validity for generated PANs in production mode")
    parser.add_argument("--prod-no-luhn", dest='prod_enforce_luhn', action='store_false', help="Do not enforce Luhn validity for generated PANs")
    parser.set_defaults(prod_enforce_luhn=True)
    parser.add_argument("--prod-ca-file", type=str, help="CA keys file to use when in production mode")
    parser.add_argument("--prod-generation-mode", choices=["random", "specific"], default="random",
                        help="When in production mode, choose random generation using dataset or specific generation")
    ...
    vuln_scan_cmd = testing_sub.add_parser("vuln-scan", help="Run vulnerability checks against generated card artifacts")
    vuln_scan_cmd.add_argument("--card-file", type=str, help="Card JSON file containing personalization data")
    vuln_scan_cmd.add_argument("--cap-file", type=str, help="CAP file path for static analysis")
    vuln_scan_cmd.add_argument("--gp-binary", type=str, help="GlobalPlatform binary path for hashing")
    vuln_scan_cmd.add_argument("--suite", type=str, help="Comma separated list of vulnerability tests to run")
    vuln_scan_cmd.add_argument("--run-pos", action="store_true", help="Execute POS regression prior to scan")
    vuln_scan_cmd.add_argument("--run-atm", action="store_true", help="Execute ATM regression prior to scan")
    vuln_scan_cmd.add_argument("--include-hsm", action="store_true", help="Include HSM command simulation in the run")
    vuln_scan_cmd.add_argument("--output", type=str, help="Write results to JSON file instead of stdout")
    ...
    realworld.add_argument("--profile", type=str,
                           help="Use predefined card profile(s) (comma separated)")
    realworld.add_argument("--list-profiles", action="store_true",
                           help="List available real-world card profiles and exit")
    realworld.add_argument("--real-keys", action="store_true",
                           help="Prefer production key descriptors instead of lab keys")
    realworld.add_argument("--variant", type=str,
                           help="Select a specific profile variant (e.g., dda, sda, mifare)")
    realworld.add_argument("--list-variants", action="store_true",
                           help="List variants available for selected profile(s)")
    realworld.add_argument("--auto-config", action="store_true",
                           help="Use defaults from central configuration without prompting")
    realworld.add_argument("--prompt-config", action="store_true",
                           help="Prompt for configurable parameters defined in the profile")
    ...
    test_group.add_argument("--test-atm", action="store_true", help="Test cards with ATM emulator after issuance")
    ...
    operator_console = sub.add_parser(
        "operator",
        help="Launch operator console for merchant, ATM/HSM, card, and cryptography workflows",
    )
    operator_console.add_argument(
        "--section",
        choices=["merchant", "atm", "cards", "crypto"],
        help="Auto-launch a specific operator section",
    )
    ...
    config_cmd = sub.add_parser("config", help="Inspect or modify GREENWIRE's central configuration")
    config_cmd.add_argument("operation", choices=["show", "set", "list", "reset"],
                            help="Configuration operation to perform")
    config_cmd.add_argument("--path", type=str,
                            help="Dot-delimited configuration path (e.g., cards.default_profile)")
    config_cmd.add_argument("--value", type=str,
                            help="Value to set (JSON string for complex values)")
    config_cmd.add_argument("--section", type=str,
                            help="Configuration section to list or reset")
    config_cmd.add_argument("--interactive", action="store_true",
                            help="Launch interactive configuration wizard")
    config_cmd.add_argument("--output", type=str,
                            help="Optional path to export configuration snapshot")
...
def run_apdu_fuzz_cli(args):
    ...
    if args.hardware:
        try:
            ...
            if hw:
                ...
                hardware_mode = True
                safe_print("✅ Using hardware reader:", r)
            else:
    ...
    if args.json_artifact:
        ...
        try:
            ...
            safe_print("📁 JSON session artifact:", json_path)
        except Exception as e:
            safe_print("❌ Failed to write JSON artifact:", e)
    return 0
...
def run_realworld_card_generation(args):
    ...
    if getattr(args, "list_profiles", False):
        ...
        if not profiles:
            print("⚠️ No card profiles available.")
            return
        print("\n📚 Available Card Profiles")
        print("-" * 70)
        for profile in profiles:
            pid = profile.get('profile_id')
            name = profile.get('display_name', pid)
            scheme = profile.get('scheme', 'n/a')
            card_type = profile.get('card_type', 'n/a')
            logging_cfg = profile.get('transaction_logging', {})
            merchant_tests = ", ".join(profile.get('merchant_test_matrix', [])) or "(none)"
            atm_tests = ", ".join(profile.get('atm_test_matrix', [])) or "(none)"
            print(f"• {pid}: {name}")
            print(f"   Scheme: {scheme.upper()} | Type: {card_type}")
            print(f"   Logging: {logging_cfg.get('mode', 'transaction_journal')} -> EF {logging_cfg.get('ef_id', 'EFTRANLOG')} ({logging_cfg.get('max_records', 0)} records)")
            print(f"   POS Matrix: {merchant_tests}")
            print(f"   ATM Matrix: {atm_tests}\n")
        return

    profile_arg = getattr(args, "profile", None)
    selected_profiles: List[str] = []
    if profile_arg:
        for pid in [p.strip() for p in profile_arg.split(',') if p.strip()]:
            if not issuer.get_card_profile(pid):
                raise ValueError(f"Unknown card profile requested: {pid}")
            selected_profiles.append(pid)

    use_real_keys = getattr(args, "real_keys", False)
    scheme = args.scheme if args.scheme != "auto" else None
    count = args.count
    card_type = args.type
    region = args.region
    cvm_method = args.cvm_method
    dda_enabled = getattr(args, "dda", True)
    risk_level = getattr(args, "risk_level", "very_low")
    floor_limit = getattr(args, "floor_limit", 50)
    cvr_settings = getattr(args, "cvr_settings", None)
    cardholder_name = getattr(args, "cardholder_name", None)
    expiry_date = getattr(args, "expiry_date", None)
    preferred_bank = getattr(args, "preferred_bank", None)
    force_bin = getattr(args, "force_bin", None)
    generate_cap = getattr(args, "generate_cap", False)
    cap_output_dir = getattr(args, "cap_output_dir", "realworld_caps") or "realworld_caps"
    test_merchant = getattr(args, "test_merchant", False)
    test_atm = getattr(args, "test_atm", False)
    production_ready = getattr(args, "production_ready", False)
    output_file = getattr(args, "output_file", None)
    output_format = getattr(args, "output_format", "json")

    if production_ready:
        dda_enabled = True
        risk_level = "low" if risk_level == "very_low" else risk_level
        use_real_keys = True
    ...
    print(f"  • CVM method: {cvm_method}")
    print(f"  • Risk level: {risk_level}, floor limit {floor_limit}")
    print(f"  • DDA Enabled: {'Yes' if dda_enabled else 'No'}")
    print(f"  • Key preference: {'Production descriptors' if use_real_keys else 'Lab/test keys'}")
    print(f"  • Generate CAP files: {'Yes' if generate_cap else 'No'}")
    print(f"  • Auto POS tests: {'Yes' if test_merchant else 'No'} | Auto ATM tests: {'Yes' if test_atm else 'No'}")
    if output_file:
        print(f"  • Aggregate output -> {output_file} ({output_format})")

    print("\n🏦 Generating Real-World EMV Cards...")

    generation_plan: List[Optional[str]] = []
    if selected_profiles:
        if len(selected_profiles) == 1:
            generation_plan = selected_profiles * count
        else:
            while len(generation_plan) < count:
                generation_plan.extend(selected_profiles)
            generation_plan = generation_plan[:count]
    else:
        generation_plan = [None] * count
    ...
    def _append_log(card_obj: Dict[str, Any], operation: str, result: Any) -> None:
        ...
        card_obj.setdefault('transaction_log_records', []).append(record)

    for index, profile_id in enumerate(generation_plan, start=1):
        # Prepare overrides for each card
        overrides = {}
        if getattr(args, 'cvm_method', None):
            overrides['cvm_method'] = args.cvm_method
        terminal_profile_id = getattr(args, 'terminal_profile', None) or (profile_id and isinstance(profile_id, dict) and profile_id.get('default_terminal_profile'))
        if terminal_profile_id:
            overrides['terminal_profile_id'] = terminal_profile_id
        terminal_overrides = getattr(args, 'terminal_overrides', None)
        if terminal_overrides:
            overrides['terminal_overrides'] = terminal_overrides
        # ...existing code for card generation using overrides...
        try:
            if profile_id:
                profile = issuer.get_card_profile(profile_id)
                current_scheme = profile.get('scheme', scheme or 'visa')
                overrides = {
                    'cvm_method': cvm_method,
                    'risk_level': risk_level,
                    'floor_limit': floor_limit,
                ...
                card = issuer.generate_card_from_profile(
                    profile_id,
                ...
                current_scheme = scheme if scheme else random.choice(["visa", "mastercard", "amex"])
                ...
                card.setdefault('profile_id', f"{current_scheme}_ad_hoc")
                card.setdefault('profile_name', f"{current_scheme.upper()} {card_type.title()} (Ad-hoc)")
                card.setdefault('profile_description', 'Generated without predefined template.')
                card['supports_randomized_data'] = True
                card['supports_production_data'] = use_real_keys
                card.setdefault('key_management', {
                    'selected_option': 'production_keys' if use_real_keys else 'test_keys',
                    'options': {
                        'test_keys': {'notes': 'Auto-generated at issuance'},
                        'production_keys': {'notes': 'Operator must import keys via crypto keys import'}
                    },
                    'requires_operator_action': use_real_keys
                })

            used_profiles.add(card.get('profile_id'))

            print(f"\n💳 [{index}/{count}] {card.get('profile_name')} ({card.get('scheme')})")
            print(f"  • Card Number: {card['card_number']}")
            print(f"  • Cardholder: {card['cardholder_name']}")
            print(f"  • Expiry: {card['expiry_date']}")
            print(f"  • Issuer: {card['issuer_bank']} | Routing #: {card['routing_number']}")
            print(f"  • CVM: {card['cvm_method']} | DDA: {'Enabled' if card['dda_enabled'] else 'Disabled'}")
            logging_cfg = card.get('transaction_logging', {})
            print(f"  • Logging: {logging_cfg.get('mode')} -> {logging_cfg.get('ef_id')} (max {logging_cfg.get('max_records')} records)")
            print(f"  • POS Matrix: {', '.join(card.get('merchant_test_matrix', []))}")
            print(f"  • ATM Matrix: {', '.join(card.get('atm_test_matrix', []))}")

            pdol_template = card.get('pdol_template', [])
            if pdol_template:
                pdol_summary = ', '.join(f"{entry['tag']}({entry['length']})" for entry in pdol_template)
                print(f"  • PDOL Template: {pdol_summary}")
            cdol1_template = card.get('cdol1_template', [])
            if cdol1_template:
                cdol_summary = ', '.join(f"{entry['tag']}({entry['length']})" for entry in cdol1_template)
                print(f"  • CDOL1 Template: {cdol_summary}")

            # Persist card data
            filename = issuer.save_card_to_file(card)
            print(f"  • Saved to: {filename}")

            # CAP generation placeholder
            cap_filename: Optional[str] = None
            if generate_cap:
                print(f"  • CAP generation pipeline (placeholder): {cap_output_dir}")
                os.makedirs(cap_output_dir, exist_ok=True)
                cap_filename = os.path.join(cap_output_dir, f"{card.get('profile_id', card['scheme'])}_{index}.cap")
                print("    - CAP build currently not implemented. Use tools/create_static_bundle.py if needed.")

            # Automated POS / ATM tests
            card.setdefault('test_results', {})
            if test_merchant:
                print("  • POS Regression: running merchant emulator...")
                try:
                    from modules.merchant_emulator import MerchantEmulator
                    emulator = MerchantEmulator(verbose=True)
                    merchant_summary = emulator.purchase(amount_cents=1000)
                    card['test_results']['merchant'] = merchant_summary
                    print(json.dumps(merchant_summary, indent=2))
                    _append_log(card, 'merchant_purchase', merchant_summary)
                except Exception as exc:
                    print(f"    ⚠️ Merchant emulator failed: {exc}")

            if test_atm:
                print("  • ATM Regression: running ATM emulator...")
                try:
                    from modules.atm_emulator import ATMEmulator
                    atm_emulator = ATMEmulator(verbose=True)
                    atm_summary = atm_emulator.withdraw(amount_cents=2000)
                    card['test_results']['atm'] = atm_summary
                    print(json.dumps(atm_summary, indent=2))
                    _append_log(card, 'atm_withdrawal', atm_summary)
                except Exception as exc:
                    print(f"    ⚠️ ATM emulator failed: {exc}")

            if cap_filename and os.path.exists(cap_filename):
                print(f"  • CAP ready at {cap_filename} (manual install required)")

            cards.append(card)
            aggregate_records.append(card)

        except Exception as gen_error:
            print(f"\n❌ Error generating card {index}: {gen_error}")

    if output_file and aggregate_records:
        try:
            if output_format == 'json':
                with open(output_file, 'w', encoding='utf-8') as f:
                    json.dump(aggregate_records, f, indent=2, default=str)
            elif output_format == 'csv':
                import csv
                fieldnames = ['card_number', 'cardholder_name', 'expiry_date', 'scheme', 'profile_id', 'profile_name', 'risk_level', 'floor_limit', 'supports_randomized_data', 'supports_production_data']
                with open(output_file, 'w', newline='', encoding='utf-8') as f:
                    writer = csv.DictWriter(f, fieldnames=fieldnames)
                    writer.writeheader()
                    for card in aggregate_records:
                        writer.writerow({
                            ...
                            'floor_limit': card.get('floor_limit'),
                            'supports_randomized_data': card.get('supports_randomized_data'),
                            'supports_production_data': card.get('supports_production_data'),
                        })
            else:
                with open(output_file, 'w', encoding='utf-8') as f:
                    for card in aggregate_records:
                        f.write(f"{card.get('card_number')} | {card.get('profile_name')} | PDOL entries: {len(card.get('pdol_template', []))}\n")
            print(f"\n📁 Wrote aggregate output to {output_file} ({output_format})")
        except Exception as output_error:
            print(f"⚠️ Failed to write aggregate output: {output_error}")
    ...
    print("• PDOL/CDOL templates embedded for terminal testing")
    print("• Transaction journaling configured for on-card logging")
    print("• Merchant POS matrices cover contact/contactless, fallback, and CVM variants")
    print("• ATM matrices include withdrawal, balance, PIN management, and multi-currency where applicable")
    if test_merchant or test_atm:
        print("• Emulator regressions appended to in-card transaction logs")
...
def show_nfc_menu():
    """NFC/RFID Standards & Communication submenu - comprehensive hardware operations."""
    print("\n" + "="*60)
    print("   📡 NFC/RFID Standards & Communication")
    print("="*60)
    print("1. 📱 NFC Standards (ISO/IEC 14443, 15693, 18000)")
    print("2. 🏷️  RFID Standards (LF/HF/UHF - ISO/IEC 11784/11785, etc.)")
    print("3. � Hardware Selection & Setup")
    print("4. � Scan & Detect Cards/Tags")
    print("5. � Read/Write Operations")
    print("6. 📊 Protocol Analysis & Testing")
    print("7. 🔒 Security & Vulnerability Testing")
    print("")
    print("Note: All operations are RFC/ISO compliant")
    print("0. Back to main menu")
    print()

    choice = input("Enter your choice: ").strip()
    if choice == "1":
        return show_nfc_standards_menu()
    elif choice == "2":
        return show_rfid_standards_menu()
    elif choice == "3":
        return show_hardware_selection_menu()
    elif choice == "4":
        return show_scan_menu()
    elif choice == "5":
        return show_read_write_menu()
    elif choice == "6":
        return show_protocol_analysis_menu()
    elif choice == "7":
        return show_security_testing_menu()
    elif choice == "0":
        return None
    else:
        print("Invalid choice. Please try again.")
        return show_nfc_menu()


# Note: Orphaned Android NFC verification code (approx. 850 lines) was removed from here
# The code was malformed with mismatched indentation and orphaned except/elif blocks
# If this functionality is needed, implement it properly in a separate module or fix in show_nfc_menu()
...
def run_gp(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        if result.stderr:
            print("--- stderr ---")
            print(result.stderr)
        # If this was a CAP install and succeeded, offer advanced EMV/ATR reporting and POS/ATM/fuzzer test
        if any(x in gp_args for x in ["--install", "--load", "--create"]) and result.returncode == 0:
            print("\n✅ CAP install/GP operation completed successfully!")
            # Try to read ATR, card type, EMV details
            try:
                from menu_implementations import SmartCardManager
                from core.emv_processor import parse_atr, parse_emv_tags, parse_pdol_cdol, parse_cvm_ttq
                mgr = SmartCardManager()
                reader = getattr(args, 'reader', None)
                print("\n🔎 Reading ATR and card details...")
                atr = mgr.get_atr(reader=reader)
                print(f"   ATR: {atr}")
                card_type = mgr.detect_card_type(atr)
                print(f"   Card Type: {card_type}")
                emv_data = mgr.read_emv_data(reader=reader)
                if emv_data:
                    tags = parse_emv_tags(emv_data)
                    print(f"   EMV Tags: {tags}")
                    pdol, cdol = parse_pdol_cdol(tags)
                    print(f"   PDOL: {pdol}")
                    print(f"   CDOL: {cdol}")
                    ttq, cvm = parse_cvm_ttq(tags)
                    print(f"   TTQ: {ttq}")
                    print(f"   CVM: {cvm}")
                    if cvm and 'offline_pin' in cvm.lower():
                        print("   ⚠️  Offline PIN required for this card. Please enter PIN at terminal.")
                else:
                    print("   (No EMV data found or card not EMV-compliant)")
            except Exception as e:
                print(f"   (Could not read ATR/EMV details: {e})")
            # Offer to run merchant, ATM, or fuzzer test
            try:
                print("\nWould you like to test this card with a merchant, ATM emulator, or fuzzer?")
                print("  1. Merchant POS test\n  2. ATM withdrawal test\n  3. Fuzzer test\n  4. Skip")
                choice = input("Select option (1-4): ").strip()
                if choice == "1":
                    from modules.merchant_emulator import MerchantEmulator
                    emulator = MerchantEmulator(reader=reader, verbose=True)
                    amount = input("Enter purchase amount (e.g., 9.99): ").strip()
                    try:
                        amount_cents = int(float(amount) * 100)
                    except Exception:
                        amount_cents = 999
                    summary = emulator.purchase(amount_cents=amount_cents)
                    print(summary)
                elif choice == "2":
                    from modules.atm_emulator import ATMEmulator
                    emulator = ATMEmulator(reader=reader, verbose=True)
                    amount = input("Enter withdrawal amount (e.g., 20.00): ").strip()
                    try:
                        amount_cents = int(float(amount) * 100)
                    except Exception:
                        amount_cents = 2000
                    summary = emulator.withdraw(amount_cents=amount_cents)
                    print(summary)
                elif choice == "3":
                    print("\nLaunching fuzzer against this card...")
                    from greenwire.core.apdu_fuzzer import run_emv_fuzzing_session
                    try:
                        run_emv_fuzzing_session(reader=reader)
                    except Exception as e:
                        print(f"   (Fuzzer failed: {e})")
                else:
                    print("Skipping POS/ATM/fuzzer test.")
            except Exception as e:
                print(f"   (Could not run POS/ATM/fuzzer test: {e})")
    except FileNotFoundError:
    ...
    if args.easycard_command == "list-ca":
        ...
        try:
            ...
            if args.method == "random":
                ...
                prod_opts = getattr(args, 'production_options', {}) if getattr(args, 'operator_mode', 'simulation') == 'production' else {}
                iin_override = prod_opts.get('iin_override')
                ...
                for idx in range(args.count):
                    ...
                    if iin_override:
                        for c in cards:
                            ...
                            if not num:
                                continue
                            if len(num) > len(iin_override) + 1:
                                middle = num[len(iin_override):-1]
                            else:
                                middle = num[len(iin_override):]
                            new_base = iin_override + middle
                            try:
                                cg = CardGenerator()
                                check = cg._calculate_luhn_check_digit(new_base)
                                c['card_number'] = new_base + str(check)
                            except Exception:
                                pass
            ...
            if args.method == "certificate":
                ...
                if os.path.exists(ca_file):
                    cards = card_gen.generate_card('visa', args.count)
                    print(f"\n✅ Generated {len(cards)} certificate-based cards using {ca_file}")
                else:
            ...
            if args.method == "manual":
                prefix = args.prefix
                if getattr(args, 'operator_mode', 'simulation') == 'production':
                    prod_opts = getattr(args, 'production_options', {})
                    if not prefix:
                        prefix = prod_opts.get('iin_override') or input("Enter card prefix (e.g., 4111): ")
                else:
                    if not prefix:
                        prefix = input("Enter card prefix (e.g., 4111): ")
                if len(prefix) < 4:
                ...
                for c in cards:
                    ...
                    if num and len(num) > len(prefix) + 1:
                        middle = num[len(prefix):-1]
                        try:
                            cg = CardGenerator()
                            cd = cg._calculate_luhn_check_digit(prefix + middle)
                            c['card_number'] = prefix + middle + str(cd)
                        except Exception:
                            pass
...
def run_operator_console(args: argparse.Namespace) -> None:
    """Launch the operator console for quick access to common suites."""
    try:
        from menu_handlers import operator_console_interactive, operator_console_run_section
    except ImportError as exc:
        print(f"❌ Operator console unavailable: {exc}")
        return

    section = getattr(args, "section", None)
    if section:
        try:
            result = operator_console_run_section(section)
    ...
    operator_console_interactive()

# ---------------------------------------------------------------------------
# Encrypted input.json handling (card issuance personalization seeds)
# ---------------------------------------------------------------------------
def _decrypt_encrypted_input(path: str, passphrase: Optional[str]) -> Optional[dict]:
    """Decrypt an encrypted input.json style file.

    Expected JSON wrapper fields:
      kdf (pbkdf2-sha256), salt, iterations, cipher (aes-256-gcm), nonce, tag, payload
    Returns decrypted dict or None on failure.
    """
    logger = get_logger()
    try:
        ...
        print(f"Decryption prerequisites missing: {e}")
        return None
    if not _os.path.isfile(path):
        print(f"Encrypted input file not found: {path}")
        return None
    try:
        with open(path, 'r', encoding='utf-8') as fh:
            blob = _json.load(fh)
    except Exception as e:
        print(f"Failed to parse encrypted input file: {e}")
        return None
    required = {"kdf", "salt", "iterations", "cipher", "nonce", "tag", "payload"}
    if not required.issubset(blob.keys()):
        print("Encrypted input file missing required fields; skipping.")
        return None
    if blob["cipher"].lower() != "aes-256-gcm" or blob["kdf"].lower() != "pbkdf2-sha256":
        print("Unsupported encryption parameters.")
        return None
    if passphrase is None:
        if _sys.stdin.isatty():
            passphrase = _getpass.getpass("Passphrase for encrypted input: ")
        else:
            print("No passphrase provided and no TTY available.")
            return None
    try:
        salt = binascii.unhexlify(blob["salt"])
        nonce = binascii.unhexlify(blob["nonce"])
        tag = binascii.unhexlify(blob["tag"])
        payload = binascii.unhexlify(blob["payload"])
    except Exception as e:
        print(f"Invalid hex in encrypted input file: {e}")
        return None
    try:
        key = hashlib.pbkdf2_hmac("sha256", passphrase.encode(), salt, int(blob["iterations"]), dklen=32)
        aesgcm = AESGCM(key)
        start_time = time.perf_counter()
        plaintext = aesgcm.decrypt(nonce, payload + tag, None)
        duration_ms = (time.perf_counter() - start_time) * 1000.0
        logger.info(
            f"Decrypted encrypted artifact '{path}' in {duration_ms:.3f} ms",
            "decrypt_artifact",
        )
        return _json.loads(plaintext.decode('utf-8'))
    except Exception as e:
        duration_ms = (time.perf_counter() - start_time) * 1000.0 if 'start_time' in locals() else 0.0
        logger.warning(
            f"Failed to decrypt artifact '{path}' after {duration_ms:.3f} ms: {e}",
            "decrypt_artifact",
        )
        print(f"Failed to decrypt input file: {e}")
        return None

def process_encrypted_issuer_input(args: argparse.Namespace):
    """Return decrypted personalization seed dict for issuer role or None."""
    path = getattr(args, 'encrypted_input', None)
    if not path:
        return None
    passphrase = getattr(args, 'input_passphrase', None)
    data = _decrypt_encrypted_input(path, passphrase)
    if data is None:
        return None
    print("Decrypted input.json payload loaded (sensitive values redacted).")
    redacted = {k: ('<redacted>' if any(x in k.lower() for x in ['key', 'secret', 'pin']) else v) for k, v in data.items()}
    for k, v in redacted.items():
        if isinstance(v, (dict, list)):
            print(f"  {k}: <structured>")
        else:
            print(f"  {k}: {v}")
    return data


def handle_easycard_command(args):
    if args.easycard_command == "list-profiles":
        ...
        try:
            ...
            profiles_argument = getattr(args, "profile", None)
            profile_ids = [p.strip() for p in profiles_argument.split(",") if p.strip()] if profiles_argument else []
            terminal_overrides: Dict[str, Any] = {}
            if getattr(args, "terminal_overrides", None):
                try:
                    terminal_overrides = json.loads(args.terminal_overrides)
                except json.JSONDecodeError as exc:
                    print(f"❌ Invalid JSON for --terminal-overrides: {exc}")
                    return

            if getattr(args, "list_profiles", False):
                print("📚 Available Real-World Card Profiles:")
                for profile in issuer.list_card_profiles():
                    variants = profile.get("authentication_variants", [])
                    print(f" - {profile.get('profile_id')} :: {profile.get('display_name', 'Unnamed')} [{profile.get('scheme', 'unknown')}]")
                    if variants:
                        print(f"     Variants: {', '.join(variants)}")
                return

            if getattr(args, "list_terminal_profiles", False):
                print("🏧 Available Terminal Profiles:")
                for terminal_profile in issuer.list_terminal_profiles():
                    display = terminal_profile.get('display_name', 'Unnamed')
                    terminal_type = terminal_profile.get('terminal_type', 'N/A')
                    print(f" - {terminal_profile.get('profile_id')} :: {display} [{terminal_type}]")
                    caps = terminal_profile.get('capabilities', {})
                    if caps:
                        enabled = [name for name, flag in caps.items() if flag]
                        if enabled:
                            print(f"     Capabilities: {', '.join(enabled)}")
                return

            if getattr(args, "list_variants", False):
                if not profile_ids:
                    print("❌ --profile is required when using --list-variants")
                    return
                for profile_id in profile_ids:
                    ...
                    variants = profile.get("authentication_variants", [])
                    if variants:
                        print(f"{profile_id}: {', '.join(variants)}")
                    else:
                        print(f"{profile_id}: (no explicit variants)")
                return

            cardholder_name = getattr(args, "cardholder_name", None)
            expiry_date = getattr(args, "expiry_date", None)
            suite_defaults = configuration.get("vulnerability_scanning", {}).get("default_suite", [])
            output_records: List[Dict[str, Any]] = []

            def render_card_summary(card: Dict[str, Any], index: int, total: int) -> None:
                print(f"\nCard {index} of {total}:")
                profile_label = card.get('profile_name') or card.get('profile_id') or 'ad-hoc'
                print(f"  Profile: {profile_label}")
                print(f"  Card Number: {card.get('card_number', 'unknown')}")
                print(f"  Cardholder: {card.get('cardholder_name', 'N/A')}")
                print(f"  Expiry: {card.get('expiry_date', 'N/A')}")
                print(f"  Scheme: {card.get('scheme', 'N/A')}  |  Type: {card.get('card_type', 'N/A')}")
                print(f"  Issuer: {card.get('issuer_bank', 'N/A')}  |  Routing #: {card.get('routing_number', 'N/A')}")
                print(f"  CVM Method: {card.get('cvm_method', 'N/A')}  |  DDA Enabled: {card.get('dda_enabled')}")
                print(f"  Risk Level: {card.get('risk_level', 'very_low')}  |  Floor Limit: {card.get('floor_limit', 50)}")

                operator_opts = card.get('operator_options', {}).get('authentication_variant', {})
                if operator_opts:
                    available = operator_opts.get('available') or []
                    print(f"  Authentication Variant: {operator_opts.get('selected')} (choices: {', '.join(available)})")

                merchant = card.get('merchant_data', {})
                if merchant:
                    print(f"  Merchant ID: {merchant.get('merchant_id', 'N/A')}  |  Terminal ID: {merchant.get('terminal_id', 'N/A')}")
                    print(f"  Merchant Category: {merchant.get('merchant_category_name', merchant.get('merchant_category', 'N/A'))}")

                logging_cfg = card.get('transaction_logging', {})
                print(f"  Logging EF: {logging_cfg.get('ef_id', 'EFTRANLOG')}  |  Max Records: {logging_cfg.get('max_records', 0)}")
                terminal_profile = card.get('terminal_profile')
                if isinstance(terminal_profile, dict):
                    profile_label = terminal_profile.get('display_name') or terminal_profile.get('profile_id') or 'custom'
                    print(f"  Terminal Profile: {terminal_profile.get('profile_id', 'custom')} :: {profile_label}")
                    environment = terminal_profile.get('environment', {})
                    if environment:
                        env_summary = ", ".join(
                ...
                if card.get('communication_log'):
                    print(f"  Communication Log Entries: {len(card['communication_log'])}")
                    sample = card['communication_log'][:2]
                    for entry in sample:
                        print(f"    - {entry.get('timestamp')} {entry.get('channel')} {entry.get('operation')} -> {entry.get('status')}")

                auto_results = card.get('automatic_test_results')
                if auto_results:
                    print("  Smart Testing Results Available:")
                    for channel, result in auto_results.items():
                        if isinstance(result, dict):
                            summary = result.get('status') or result.get('plan') or result.get('enabled')
                            print(f"    {channel}: {summary}")

            def save_output(records: List[Dict[str, Any]]) -> None:
                if not records or not getattr(args, 'output_file', None):
                    return
                ...
                try:
                    if output_format == 'json':
                        ...
                        fields = [
                            ...
                            'selected_variant', 'risk_level', 'floor_limit', 'dda_enabled',
                        ]
                        ...
                        with open(output_path, 'w', encoding='utf-8') as fh:
                            for idx, record in enumerate(records, 1):
                                ...
                                fh.write("\n\n")
                    print(f"💾 Card data saved to {output_path} ({output_format})")
                except OSError as exc:
                    print(f"❌ Failed to write output file: {exc}")

            def determine_variant(profile: Dict[str, Any], available: List[str]) -> Optional[str]:
                if not available:
                    return None
                if getattr(args, 'variant', None) and args.variant in available:
                    return args.variant
                preferred = configuration.get('profiles', {}).get('preferred_variants', {}).get(
                    profile.get('variant_category', 'emv')
                )
                if preferred in available:
                    return preferred
                should_prompt = getattr(args, 'prompt_config', False) or (
                    configuration.get('profiles', {}).get('prompt_operator', True) and not getattr(args, 'auto_config', False)
                )
                if should_prompt:
                    print(f"Available variants for {profile.get('profile_id')}: {', '.join(available)}")
                    choice = input("Select variant (press Enter for default): ").strip()
                    if choice and choice in available:
                        return choice
                return available[0]

            total_cards = args.count if args.count > 0 else 1
            issued_count = 0
            ...
            if profile_ids:
                for profile_id in profile_ids:
                    ...
                    variant_choice = determine_variant(profile, available_variants)

                    overrides: Dict[str, Any] = {
                        'auth_variant': variant_choice,
                        'variant_category': profile.get('variant_category', 'emv'),
                    }
                    terminal_profile_id = getattr(args, 'terminal_profile', None) or profile.get('default_terminal_profile')
                    if terminal_profile_id:
                        overrides['terminal_profile_id'] = terminal_profile_id
                    if terminal_overrides:
                        overrides['terminal_overrides'] = terminal_overrides.copy()
                    if getattr(args, 'risk_level', None):
                        overrides['risk_level'] = args.risk_level
                    if getattr(args, 'floor_limit', None) is not None:
                        overrides['floor_limit'] = args.floor_limit
                    if getattr(args, 'cvr_settings', None):
                        overrides['cvr_settings'] = args.cvr_settings
                    if getattr(args, 'preferred_bank', None):
                        overrides['preferred_bank'] = args.preferred_bank
                    if getattr(args, 'force_bin', None):
                        overrides['force_bin'] = args.force_bin
                    if getattr(args, 'cvm_method', None):
                        overrides['cvm_method'] = args.cvm_method
                    ...
                    for _ in range(total_cards):
                        ...
                        if (args.test_merchant or args.test_atm) and not card.get('automatic_test_results'):
                            tester = SmartVulnerabilityTestCard(card)
                            tester.run_automatic_tests(
                                run_pos=bool(args.test_merchant),
                                run_atm=bool(args.test_atm),
                                include_hsm=configuration.get('hsm', {}).get('enabled', False),
                                vulnerability_suite=suite_defaults,
                            )
                            tester.persist_logs_to_card()

                        issued_count += 1
                        render_card_summary(card, issued_count, total_cards * max(1, len(profile_ids)))
                        if getattr(args, 'output_file', None):
                            output_records.append(card)

            else:
                # Generate ad-hoc cards using configuration defaults
                base_scheme = args.scheme
                if base_scheme == 'auto':
                    base_scheme = random.choice(['visa', 'mastercard', 'amex'])

                card_settings: Dict[str, Any] = {
                    'scheme': base_scheme,
                    'card_type': args.type,
                    'dda_enabled': args.dda,
                    'cvm_method': getattr(args, 'cvm_method', 'offline_pin_signature'),
                    'risk_level': getattr(args, 'risk_level', 'very_low'),
                    'floor_limit': getattr(args, 'floor_limit', 50),
                    'cvr_settings': getattr(args, 'cvr_settings', None),
                    'cardholder_name': cardholder_name,
                    ...
                    'preferred_bank': getattr(args, 'preferred_bank', None),
                    'force_bin': getattr(args, 'force_bin', None),
                    'terminal_profile_id': getattr(args, 'terminal_profile', None),
                    'terminal_overrides': terminal_overrides.copy() if terminal_overrides else None,
                }
                ...
                for _ in range(total_cards):
                    ...
                    issued_count += 1
                    render_card_summary(card, issued_count, total_cards)
                    if getattr(args, 'output_file', None):
                        output_records.append(card)
        ...
        # Install card to smart card using GlobalPlatform, then report EMV/ATR details and offer POS test
        try:
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    # --- Begin: EMV/ATR/PDOL/CDOL/CVM/TTQ/Offline PIN reporting ---
                    try:
                        from menu_implementations import SmartCardManager
                        from core.emv_processor import parse_atr, parse_emv_tags, parse_pdol_cdol, parse_cvm_ttq
                        mgr = SmartCardManager()
                        ...
                        card_type = mgr.detect_card_type(atr)
                        print(f"   Card Type: {card_type}")
                        # Try to select the applet and read EMV data
                        emv_data = mgr.read_emv_data(reader=reader)
                        if emv_data:
                            tags = parse_emv_tags(emv_data)
                            print(f"   EMV Tags: {tags}")
                        ...
                        print(f"   (Could not read ATR/EMV details: {e})")
                    # --- End: EMV/ATR/PDOL/CDOL/CVM/TTQ/Offline PIN reporting ---

                    # Offer to run merchant or ATM emulator test
                    try:
                        print("\nWould you like to test this card with a merchant or ATM emulator?")
                        print("  1. Merchant POS test\n  2. ATM withdrawal test\n  3. Skip")
                        ...
                        if choice == "1":
                            ...
                            print("Skipping POS/ATM test.")
                    except Exception as e:
                        print(f"   (Could not run POS/ATM test: {e})")
...
def run_legacy(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        if legacy_cmd == 'menu':
            ...
            print("Unknown legacy command:", legacy_cmd)
            print("Available commands: menu, convert, export, info")

    except Exception as e:
        logger.error("Legacy operation failed: %s", e)
        print("Error:", e)
...
def run_interactive_menu():
    ...
    try:
        ...
        logger.error("Error running interactive menu: %s", e, "MENU")
        print("Error running interactive menu:", e)
        return False
...
if __name__ == "__main__":
    ...
    # If early role selection occurred and user didn't supply --role, map it
    if _SELECTED_ROLE and not getattr(args, 'role', None):
        setattr(args, 'role', _SELECTED_ROLE)

    # Role-specific fast paths (can be extended later)
    if getattr(args, 'role', None) == 'merchant' and not args.subcommand:
        print("[ROLE] Merchant mode selected - launching merchant emulator quick flow")
        # Reuse emulate subcommand with defaults

        class _A:
            pass
        quick = _A()
        quick.mode = 'terminal'
        quick.wireless = False
        quick.aids = None
        quick.ca_file = None
        quick.issuer = None
        quick.dda = False
        quick.background = False
        quick.uid = None
        quick.data_file = None
        quick.verbose = False
        run_emulation(quick)
        sys.exit(0)
    if getattr(args, 'role', None) == 'atm' and not args.subcommand:
        print("[ROLE] ATM mode selected - launching ATM emulator quick flow (interactive)")
        # Minimal ATM emulator interaction
        try:
            from modules.atm_emulator import ATMEmulator
            emulator = ATMEmulator(verbose=False)
            amount = input("Enter withdrawal amount (e.g., 20.00): ").strip() or '20.00'
            summary = emulator.withdraw(int(round(float(amount) * 100)))
            ...
            print(f"ATM quick flow error: {e}")
        sys.exit(0)
    issuer_seed_payload = None
    if getattr(args, 'role', None) == 'issuer' and not args.subcommand:
        print("[ROLE] Issuer mode selected - entering card issuance console")
        issuer_seed_payload = process_encrypted_issuer_input(args)
        # Fall through to interactive menu for now
    ...
    if getattr(args, 'menu', False) or not getattr(args, 'subcommand', None):
        ...
        try:
            ...
            logger.error("Main execution error: %s", e, "MAIN")
            print("Error:", e)
            sys.exit(1)
